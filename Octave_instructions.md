...
`octave.bat --no-gui --interactive --silent --eval "STEMMUS_SCOPE_octave('path_to_config_file')"`

On a Unix system, use `octave` instead of `octave.bat`.
### Developing STEMMUS-SCOPE in Octave
Open the `STEMMUS_SCOPE_octave.m` file, either in VS Code or the Octave GUI.

#### Octave GUI
Set the workspace to the `STEMMUS_SCOPE/src` folder, and open the `STEMMUS_SCOPE_octave.m` file.
...
In the `STEMMUS_SCOPE_octave.m` file you can set the config file that should be used.
