      MODULE GLOBAL
        ...
        INTEGER, SAVE,    DIMENSION(:,:,:), POINTER ::IBOUND,IBOUNDOLD
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER ::CR,CROLD
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER ::CC,CCOLD
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER ::CV,CVOLD
      ...
      TYPE GLOBALTYPE
        ...
        INTEGER,    DIMENSION(:,:,:), POINTER ::IBOUND,IBOUNDOLD
        REAL,       DIMENSION(:,:,:), POINTER ::CR,CROLD
        REAL,       DIMENSION(:,:,:), POINTER ::CC,CCOLD
        REAL,       DIMENSION(:,:,:), POINTER ::CV,CVOLD
...
C     ------------------------------------------------------------------
     ...
     5                     DDREF,IBOUNDOLD,CROLD,CCOLD,CVOLD
...
C
      ...
      ALLOCATE (B(MXPAR),IACTIVE(MXPAR),IPLOC(4,MXPAR),
     1IPCLST(14,MXCLST),PARNAM(MXPAR),PARTYP(MXPAR),
     1INAME(MXINST))
C
...
! JCZENG 20180410  ADD A BACKUP ARRAY FOR IBOUND
      ALLOCATE (IBOUNDOLD(NCOL,NROW,NLAY),CROLD(NCOL,NROW,NLAY),
     1CCOLD(NCOL,NROW,NLAY),CVOLD(NCOL,NROW,NLAY))
...
C     ------------------------------------------------------------------
      ...
      USE Q3DMODULE,ONLY:DELT0,HP00,HP0,NPILLAR
...
C1------IF NOT FIRST TIME STEP THEN CALCULATE TIME STEP LENGTH.
      IF(NPILLAR>0)THEN
        DELT0=DELT!为Q3D准备
        !HP00=HOLD   !为Q3D准备
        IF(TOTIM>0)CALL PHREATIC_SURFACE(HP00,DBLE(HOLD))
        !IF(TOTIM==0)HP00=HNEW
        IF(TOTIM==0)CALL PHREATIC_SURFACE(HP00,HNEW)
        !HP0=HNEW    !为Q3D准备
        CALL PHREATIC_SURFACE(HP0,HNEW)
      ENDIF
...
C4------RETURN
      RETURN
      END
      SUBROUTINE GWF2BAS7FM(IGRID,ITERQ3D)
C     ******************************************************************
C     SET HCOF=RHS=0.
C     ******************************************************************
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,ONLY:HCOF,RHS,IBOUND,IBOUNDOLD,CV,CVOLD,CC,CCOLD,CR,
     1CROLD
...
C
        ...
        DEALLOCATE(GLOBALDAT(IGRID)%IBOUNDOLD) !JCZENG 20180410
        DEALLOCATE(GLOBALDAT(IGRID)%CR)
        DEALLOCATE(GLOBALDAT(IGRID)%CC)
        DEALLOCATE(GLOBALDAT(IGRID)%CV)
        DEALLOCATE(GLOBALDAT(IGRID)%CROLD)
        DEALLOCATE(GLOBALDAT(IGRID)%CCOLD)
        DEALLOCATE(GLOBALDAT(IGRID)%CVOLD)
...
C
        ...
        IBOUNDOLD=>GLOBALDAT(IGRID)%IBOUNDOLD!JCZENG 20180410
        CR=>GLOBALDAT(IGRID)%CR
        CC=>GLOBALDAT(IGRID)%CC
        CV=>GLOBALDAT(IGRID)%CV
        CROLD=>GLOBALDAT(IGRID)%CROLD
        CCOLD=>GLOBALDAT(IGRID)%CCOLD
        CVOLD=>GLOBALDAT(IGRID)%CVOLD
...
C
        ...
        GLOBALDAT(IGRID)%IBOUNDOLD=>IBOUNDOLD !JCZENG 20180410
        GLOBALDAT(IGRID)%CR=>CR
        GLOBALDAT(IGRID)%CC=>CC
        GLOBALDAT(IGRID)%CV=>CV
        GLOBALDAT(IGRID)%CROLD=>CROLD
        GLOBALDAT(IGRID)%CCOLD=>CCOLD
        GLOBALDAT(IGRID)%CVOLD=>CVOLD
