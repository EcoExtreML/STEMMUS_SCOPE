      ...
      SUBROUTINE GWF2BCF7AR(IN,ILGR,IGRID)
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     1                      CC,CV,IFREFM,BOTM,IBOUND !swm: added BOTM
      USE GWFBASMODULE,ONLY:HDRY
      USE LGRMODULE,   ONLY:HK,VK
      USE GWFBCFMODULE,ONLY:IBCFCB,IWDFLG,IWETIT,IHDWET,WETFCT,
     1                      LAYCON,LAYAVG,HY,SC1,SC2,WETDRY,CVWD,
     2                      TRPY
      USE Q3DMODULE,   ONLY:Q3DDAT
C
...
C8B-----READ PRIMARY STORAGE COEFFICIENT INTO ARRAY SC1 IF TRANSIENT.
      IF(ISS.EQ.0)THEN
        CALL U2DREL(SC1(:,:,K),ANAME(1),NROW,NCOL,KK,IN,IOUT)
!     JCZENG 2017.08.12
!     将MODFLOW的SS保存到数组SS(NPillar)里
        DO JJ=1,NCOL
          if(Q3DDAT(IGRID)%npillar<=0)exit
          DO II=1,NROW
            
            IP=IBOUND(JJ,II,1)
            IF(IP<=0)CYCLE
            Q3DDAT(IGRID)%SS(IP,K)=SC1(JJ,II,K)
          ENDDO
        ENDDO
      ENDIF
C
...
cswm  110 IF(K.EQ.NLAY) GO TO 120
  110 IF(K.NE.NLAY) THEN
        CALL U2DREL(CV(:,:,K),ANAME(4),NROW,NCOL,KK,IN,IOUT)
        IF(ILGR .NE. 0) VK(:,:,K) = 0.5*CV(:,:,K)*
     &                  (BOTM(:,:,K-1) - BOTM(:,:,K+1)) !swm: Apprx. VK. 
      ELSE IF(ILGR .NE. 0 .AND. NLAY .GT. 1) THEN
        VK(:,:,K) = VK(:,:,K-1)  !for bottom, use VK of layer above
      END IF
C
...
C8F-----AND LAYER TYPE IS 2 OR 3.
  120 IF(LAYCON(K).NE.3 .AND. LAYCON(K).NE.2) GO TO 130
      IF(ISS.EQ.0)THEN
        CALL U2DREL(SC2(:,:,KT),ANAME(5),NROW,NCOL,KK,IN,IOUT)
      
!     JCZENG 2017.08.12
!     将MODFLOW的Sy保存到数组Sy(NPillar)里
        DO JJ=1,NCOL
          IF(Q3DDAT(IGRID)%NPILLAR<=0)EXIT
          DO II=1,NROW
            IP=IBOUND(JJ,II,1)
            IF(IP<=0)CYCLE
            Q3DDAT(IGRID)%SY(IP,K)=SC2(JJ,II,KT)
          ENDDO
        ENDDO
      ENDIF
C
...
C3-----RETURN
      RETURN
      END
      SUBROUTINE GWF2BCF7FM(KITER,KSTP,KPER,ILGR,IGRID)
C     ******************************************************************
...
C1A-----IF LAYER TYPE IS NOT 1 OR 3 THEN SKIP THIS LAYER.
        IF(LAYCON(K).NE.3 .AND. LAYCON(K).NE.1) CYCLE
        KB=KB+1
C
C1B-----FOR LAYER TYPES 1 & 3 CALL SGWF2BCF7H TO CALCULATE
C1B-----HORIZONTAL CONDUCTANCES.
        CALL SGWF2BCF7H(KK,KB,KITER,KSTP,KPER,ILGR)
      ENDDO

C
! JCZENG 20180410 CONVERTING CELLS BACK TO BLOCKED DURING Q3D ITERATION.
! WHEN THE CELL BELOW IS NOT FULLY FILLED, THIS CELL SHOULD BE CONVERTED BACK TO DRY
!     IF(K<NLAY)THEN
!       DO J=1,NCOL
!         DO I=1,NROW
!           IF(HNEW(J,I,K+1)<=BOTM(J,I,K))then
!             IBOUND(J,I,K)=0
!             HNEW(J,I,K)=HDRY
!             IF(K<NLAY)CV(J,I,K)=ZERO
!             IF(K>1)CV(J,I,K-1)=ZERO
!             IF(J<NCOL)CR(J,I,K)=ZERO
!             IF(J>1)CR(J-1,I,K)=ZERO
!             IF(I<NROW)CC(J,I,K)=ZERO
!             IF(I>1)CC(J,I-1,K)=ZERO
!             HCOF(J,I,K)=ZERO
!             RHS(J,I,K)=ZERO
!           ENDIF
!           
...
C5------RETURN
      RETURN
      END
      SUBROUTINE SGWF2BCF7H(K,KB,KITER,KSTP,KPER,ILGR)
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     1                      LBOTM,CC,CV,IOUT!JCZENG,CR,HCOF,RHS
      USE GWFBASMODULE,ONLY:HDRY
...
C3A-----REACHED.
cswm      IF(K.EQ.NLAY)GO TO 2
      IF(K.EQ.NLAY)GO TO 22
      HTMP=HNEW(J,I,K+1)
      IF(IBOUND(J,I,K+1).GT.0.AND.HTMP.GE.TURNON)GO TO 9
cswm: add vertical check here      
   22 IF(ILGR.NE.0)THEN    !swm
        !HTMP=TURNON-1.
        !CALL SGWF2LGR2WETCHK(J,I,K,HTMP,0)
        !IF(HTMP.GE.TURNON)GO TO 9
      END IF
C
...
C3B-----THRESHOLD HAS BEEN REACHED.
    ...
    4 IF(I.EQ.1)GO TO 5
     ...
     1                            HTMP.GE.TURNON)GO TO 9
cswm    5 IF(I.EQ.NROW)GO TO 6
    5 IF(I.EQ.NROW)GO TO  7
      HTMP=HNEW(J,I+1,K)
      IF(IBOUND(J,I+1,K).GT.0.AND.HTMP.GE.TURNON)GO TO 9
c swm: add horizontal check here. 
    7 IF(ILGR.NE.0)THEN    !swm
        !HTMP=TURNON-1.
