...
!     将MODFLOW的SS保存到数组SS(NPillar)里
        DO JJ=1,NCOL
          ...
          DO II=1,NROW
            ...
            IP=IBOUND(JJ,II,1)
            IF(IP<=0)CYCLE
            Q3DDAT(IGRID)%SS(IP,K)=SC1(JJ,II,KK)
          ENDDO
        ENDDO
...
!           将MODFLOW的Sy保存到数组Sy(NPillar)里
            DO JJ=1,NCOL
              ...
              DO II=1,NROW
                IP=IBOUND(JJ,II,1)
                IF(IP<=0)CYCLE
                Q3DDAT(IGRID)%SY(IP,K)=SC2(JJ,II,LAYTYP(K))
              ENDDO
            ENDDO
...
C3-----RETURN
      RETURN
      END
      SUBROUTINE GWF2LPF7FM(KITER,KSTP,KPER,ILGR,IGRID)
C     ******************************************************************
...
C2------FOR EACH LAYER: IF CONVERTIBLE, CALCULATE CONDUCTANCES.
      ...
      KK=K
      IF(LAYTYP(K).NE.0)
     1    CALL SGWF2LPF7HCOND(KK,KITER,KSTP,KPER,ILGR)
  100 CONTINUE
      DO 101 K=1,NLAY
      KK=K
      IF(K.NE.NLAY) THEN
...
C5------CALCULATE HOR. CONDUCTANCE(CR AND CC) FOR CONSTANT T LAYERS.
      DO 70 K=1,NLAY
      KK=K
      IF(LAYTYP(K).EQ.0) CALL SGWF2LPF7HCOND(KK,0,0,0,ILGR)
   70 CONTINUE
C
...
C
      RETURN
      END
      SUBROUTINE SGWF2LPF7HCOND(K,KITER,KSTP,KPER,ILGR)
C     ******************************************************************
...
C2------IF LAYER IS WETTABLE CONVERT DRY CELLS TO WET WHEN APPROPRIATE.
     ...
     2      IHDCNV,NCNVRT,ICNVRT,JCNVRT,ACNVRT,ILGR)
C
