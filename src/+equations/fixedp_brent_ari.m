function [b, err2, fcounter] = fixedp_brent_ari(func, x0, corner, tolFn, verbose)
    ...
    if any(not_bracketting_zero)
        ...
        if any(use_x1)
            ...
            a(swap_to_a) = b(swap_to_a);
            err1(swap_to_a) = err2(swap_to_a);
            % then put the new contrapoint into 'b'
            b(use_x1) = x1(use_x1);
            err2(use_x1) = err_x1(use_x1);
        end
    ...
    while any(err_outside_tol)
        ...
        if all(abs(err_s) < tolFn)
            ...
            if any(a_into_b)
                ...
                b(a_into_b) = a(a_into_b);
                err2(a_into_b) = err1(a_into_b);
            end
            ...
            if any(b_into_a)
                ...
                c(b_into_a) = a(b_into_a);
                err3(b_into_a) = err1(b_into_a);
                a(b_into_a) = b(b_into_a);
                err1(b_into_a) = err2(b_into_a);
            end
            % now copy s into a or b
            if any(err_s_is_best)
                b(err_s_is_best) = s(err_s_is_best);
                err2(err_s_is_best) = err_s(err_s_is_best);
            end
            err_s_not_best = ~err_s_is_best & err_outside_tol;
            if any(err_s_not_best)
                a(err_s_not_best) = s(err_s_not_best);
                err1(err_s_not_best) = err_s(err_s_not_best);
    ...
    if verbose
        fprintf('Brent p/q preserving C. iterations: %d; fcalls: %d; xval: %0.5g\n', counter, fcounter, b);
    end
