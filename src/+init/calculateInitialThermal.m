function calculateInitialThermal()
    ...
    for j=1:SoilConstants.totalNumberOfElements % NL  
        ...
        JohansenThermalConductivity.RHo_bulk(j)=(1-Theta_s(j))*2.7*1000;         % Unit g.cm^-3
        JohansenThermalConductivity.TCON_dry(j)=(0.135*RHo_bulk(j)+64.7)/(2700-0.947*RHo_bulk(j));   % Unit W m-1 K-1 ==> j cm^-1 s^-1 Cels^-1
        ...
        TCON_sa=7.7;Theta_sa(j)=SoilVariables.VPER(j,1)/(1-SoilVariables.POR(j));TCON_slt=2.74;Theta_slt(j)=SoilVariables.VPER(j,2)/(1-SoilVariables.POR(j));TCON_cl=1.93;Theta_cl(j)=SoilVariables.VPER(j,3)/(1-SoilVariables.POR(j)); % thermal conductivities of soil sand, silt and clay, unit  W m-1 K-1
