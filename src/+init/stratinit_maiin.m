        ...
        for ML=1:NL
            ...
            if abs(InitLnth(ML)-InitND5)<1e-10
                for MN=1:(ML+1)
                    ...
                    XCAP(MN)=Theta_r(MN)+(Theta_s(MN)-Theta_r(MN))/(1+abs(Alpha(MN)*(-336))^n(MN))^m(MN);
                        ...
                        Theta_s(MN)=Theta_s_min(MN)*(1-XSOC(MN))+XSOC(MN)*Theta_soc;  %ERROR
            ...
            if abs(InitLnth(ML))<1e-10
                for MN=Dmark:(NL+1)
                    ...
                    h(MN)=(Inith1+(MN-Dmark+1)*(Inith0-Inith1)/(ML+2-Dmark)); % ERROR
                    IH(MN-1)=1;
                end
            end
        end
...
for MN=1:NN
 ...
 if h_frez(MN)<=h(MN)-Phi_s(J)
    ...
    if MN<NN
    XWRE(MN,1)=0; %ERROR comes from SOIL1
    XWRE(MN,2)=0;
    end
%     XK(MN)=0.0425;
end
