...
if ~isempty(tts_file)
    ScopeParameters(51).Val      = load([path_input,Dataset_dir,'/',tts_file]);
else
...
%% 2. Radiation 
if ~isempty(Rin_file)
    ScopeParameters(30).Val           = load([path_input,Dataset_dir,'/',Rin_file]);
else 
    ScopeParameters(30).Val       = ScopeParameters(30).Val*ones(size(t_));
end
if ~isempty(Rli_file)
    ScopeParameters(32).Val           = load([path_input,Dataset_dir,'/',Rli_file]);
else
...
if ~isempty(SMC_file)
    ScopeParameters(54).Val          = load([path_input,Dataset_dir,'/',SMC_file]);
end
