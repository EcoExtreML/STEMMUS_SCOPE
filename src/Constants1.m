...
DURTN=60*30*2*24*60;     % Duration of simulation period;                                  
...
Delt_t=86400*0.00001;                   % Duration of time step [Unit of second]                       
...
%> 
%>DTheta_LLh ETCON CHK KLhBAR DTDBAR IS IH XK XOLD XWRE ZETA
% hhh=zeros();
global STMHPLR0 STMHPLR1N STMZTB0 STMZTB1 STMRrN STMRrO
global TP_gg TP_gOLD TTTt TTOLD ThOLD THH IBOTSTM IPSTM TSim_Theta TSim_Temp
NumMeas=200;
% NPILR=11;
% THH=zeros(NN+1,NumMeas,NPILR);
% ThOLD=zeros(NN+1,NumMeas,NPILR);%(1:NN+1,KTN,IP0STm)=hOLD(1:NN+1).*0;
% % TTOLD=zeros(NN,NumMeas,NPILR);%(1:NN,KTN,IP0STm)=TOLD(1:NN).*0;
% % TTTt=zeros(NN,NumMeas,NPILR);%(1:NN,KTN,IP0STm)=TT(1:NN).*0;
% TP_gOLD=zeros(NN,NumMeas,NPILR);%(1:NN,KTN,IP0STm)=P_gOLD(1:NN).*0;
% TP_gg=zeros(NN,NumMeas,NPILR);%(1:NN,KTN,IP0STm)=P_gg(1:NN).*0;

% TSim_Theta=zeros(NL,NumMeas,NPILR);%(1:NL,KTN,IP0STm)=Theta_LL(NL:-1:1,1).*0;
% TSim_Temp=zeros(NL,NumMeas,NPILR);%(1:NL,KTN,IP0STm)=TT(NL:-1:1).*0;
% STMHPLR0=zeros(NumMeas,NPILR);%(KTN,IP0STm)=HPILR0.*0;
% STMHPLR1N=zeros(NumMeas,NPILR);%(KTN,IP0STm)=HPILR1N.*0;
% STMZTB0=zeros(NumMeas,NPILR);%(KTN)=ZTB0G.*0;
% STMZTB1=zeros(NumMeas,NPILR);%(KTN)=ZTB1G.*0;
% STMRrN=zeros(NumMeas,NPILR);%(KTN,IP0STm)=RrNGG.*0;
% STMRrO=zeros(NumMeas,NPILR);%(KTN,IP0STm)=RrOGG.*0;
% IBOTSTM=zeros(NumMeas,NPILR);%(KTN,IP0STm)=IBOT.*0;
% IPSTM=zeros(NumMeas,NPILR);%(KTN,IP0STm)=IP0STm.*0;
%         
XK=zeros(1,mL);
XOLD=zeros(1,mL);
KLTBAR=zeros(1,mL);
...
SaturatedK=[106.1]./86400;%[2.67*1e-3  1.79*1e-3 1.14*1e-3 4.57*1e-4 2.72*1e-4];      %[2.3*1e-4  2.3*1e-4 0.94*1e-4 0.94*1e-4 0.68*1e-4] 0.18*1e-4Saturation hydraulic conductivity (cm.s^-1);
SaturatedMC=[0.41];                              % 0.42 0.55 Saturated water content;
Coefficient_n=[1.89];                            %1.2839 1.3519 1.2139 Coefficient in VG model;
Coefficient_Alpha=[0.075];                   % 0.02958 0.007383 Coefficient in VG model;
ResidualMC=[0.065];                               %0.037 0.017 0.078 The residual water content of soil;
...
RHO_bulk=1.37;                                              % Bulk density of sand (g.cm^-3);
