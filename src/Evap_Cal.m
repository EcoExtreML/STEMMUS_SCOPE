function [Rn_SOIL, Evap, EVAP, Trap, r_a_SOIL, Srt] = Evap_Cal(InitialValues, ForcingData, SoilVariables, KT, RWU, lEstot, lEctot)
    ...
    SR = calculateSurfaceResistance(Rn, LAI, Theta_LL);
    ...
    if ModelSettings.rwuef == 1
        % TODO issue unused code below
        % calculate alpha_h
        alpha_h = calculateAlpha_h(KT, Tp_t, SoilVariables.hh);

        % calculate bx
        bx = calculateBx(InitialValues);

        % root zone water uptake
        % calculate Trap_1
        Trap_1 = calculateTrap_1(Tp_t, bx, alpha_h, SoilVariables.TT);
    ...
    if lEctot < 1000 && lEstot < 800 && lEctot > -300 && lEstot > -300 && any(SoilVariables.TT > 5)
        lambda1      = (2.501 - 0.002361 * Ta) * 1E6;
        lambda2      = (2.501 - 0.002361 * SoilVariables.Tss) * 1E6;
        Evap(KT) = lEstot / lambda2 * 0.1; % transfer to second value unit: cm s-1
        EVAP(KT, 1) = Evap(KT);
        Tp_t = lEctot / lambda1 * 0.1; % transfer to second value
