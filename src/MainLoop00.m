...
%   IP0STm=1
% if ~exist('itlevel1','var') || ~exist('itlevel2','var') || isempty(itlevel1) || isempty(itlevel2)

if IP0STm<=5 %&& itlevel1==0
...
%     end
    ...
    [KPILLAR]=KPILR_Cal(BOTm,XElemnt,IP0STm);
    % SYSTG=0.37;
    % SSSTG=0;
    % DELTT=0;
    RrOGG=0;
...
%         hh(1:NN)=THH(1:NN,KTN-1,IP0STm);
    ...
    for i=1:1:15000                        % Notice here: In this code, the 'i' is strictly used for timestep loop and the arraies index of meteorological forcing data.
        ...
        [HPILLAR,IBOT]=TIME_INTERPOLATION(TEND1,TTIME*86400,IP0STm,BOTm,HPILR1N,HPILR0,TIME,TTEND*86400,XElemnt,NN,Q3DF,ADAPTF);
        hBOT(KT)=HPILLAR;
        IBOTM(KT)=IBOT;
        %%%%%%%%%%%%%%%%%%%%%%%%%%
        if IRPT1==0 && IRPT2==0
        ...
        if IRPT1==0 && IRPT2==0
            ...
            if (TEND1-TIME)<1E-3
                ...
                [RrNGG,RrOGG,ZTB1G]=HYDRUS1RECHARGE(TEND1/86400,TTIME,ITERQ3D,IP0STm,IGRID,Q3DF,THETA0,ICTRL0,STheta_LL,TIME/86400,...
                    QS,Delt_t/86400,TTEND,ZTB0G,Shh,KPILLAR,BOTm,RrNG,SYSTG,SSSTG,XElemnt,KTN);
                %     end
                %         for MN=1:NN
                %             hOLD(MN)=h(MN);
                %             h(MN)=hh(MN);
