function [KT,TIME,Delt_t,tS,NBChh,IRPT1,IRPT2,Delt_t0]=TimestepCHK(Delt_t0,NBCh,BCh,NBChh,DSTOR,DSTOR0,EXCESS,QMT,Precip,Evap,hh,IRPT1,NN,SAVEhh,SAVETT,TT_CRIT,T0,TT,T,EPCT,h,T_CRIT,xERR,hERR,TERR,Theta_LL,Theta_L,Theta_UU,Theta_U,KT,TIME,Delt_t,NL,Thmrlefc,NBChB,NBCT,NBCTB,tS,uERR)
...
if NBCh==1
    [KT,TIME,Delt_t,IRPT1,IRPT2,tS,Delt_t0]=CnvrgnCHK(xERR,hERR,TERR,Theta_LL,Theta_L,Theta_UU,Theta_U,hh,h,TT,T,KT,TIME,Delt_t,NL,NN,Thmrlefc,NBCh,NBChB,NBCT,NBCTB,tS,uERR,SAVEhh,SAVETT);
elseif NBCh==3
...
%         CnvrgnCHK
        ...
        if DSTOR0 <= 1e-8 || (-QMT(KT)-Precip(KT)+Evap(KT))<=0 
           NBChh=2;                               
           if IRPT1==1
               IRPT1=0;IRPT2=1;
            ...
            Delt_t=DSTOR0/(-QMT(KT)-Precip(KT)+Evap(KT));
            tS=tS+1;
            return
        end
    end 
...
%             CnvrgnCHK                              % does not, proceed to find the new Delt_t. If it does, then repeat the time step with NBChh equal to 1.
        ...
        if abs(QMT(KT))<=abs(BCh)  %-------------------> NBChh is equal to 1. This means that the potential flux was thought to be excessive, and a specified head
