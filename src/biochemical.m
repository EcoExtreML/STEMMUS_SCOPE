function biochem_out = biochemical(biochem_in, sfactor, Ci_input)
	...
	% ---------------- MLROC --------------
    % ZS 2023-09-05, we use Kf=0.1, Kd=0.9, Kdf=Kd/Kf=9, Liu et al. 2022, RSE
    % This leads to maximum photochemical yield po0=(Kp/(Kf+Kd+Kp)=0.8, for Kp=4)
    Kf          = 0.1;             % []           rate constant for fluorescence
    Kd          = 0.9;             % []           rate constant for thermal deactivation at Fm
    %Kd         = max(0.8738,  0.0301*(T-273.15)+ 0.0773);																			  
    ...
    % ----------------- ZSu (1 Sep 2023)--------------------
    % Here we replace the potential electron transport rate (Je) with
    % actual LET by PSII calcualted by the mechanic light response (MLR)
    % model (Gu et al., 2019, nph) and an Open-Close redox model (Gu et
    % al., 2023, PCE).

    % Je          = 0.5 * po0 .* Q .* fPAR;          % potential electron transport rate (JAK: add fPAR)
%     [Je, q_PSII] = MLROC(J_PSII);
    jA = actualElectronTransportRate();
