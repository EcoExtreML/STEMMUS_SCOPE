function [ETCON,EHCAP,TETCON,EfTCON,ZETA]=EfeCapCondUncp(HCAP,SF,TCA,GA1,GA2,GB1,GB2,HCD,ZETA0,CON0,PS1,PS2,XWILT,XK,TT,NL,POR,Theta_LL,DRHOVT,L,D_A,RHOV,Theta_V,Theta_II,TCON_dry,Theta_s,XSOC,ThermCond,TPS1,TPS2,TCON0,TCON_s,FEHCAP,RHOI,RHOL,ETCON,EHCAP,TETCON,EfTCON,ZETA)
global TCON
MN=0;TCON(1)=1.37e-3*4.182;
for ML=1:NL
    for ND=1:2
        MN=ML+ND-1;
        J=ML;%J=IS(ML);
        ...
        TCON(2)=0;%TCA+D_A(MN)*L(MN)*DRHOVT(MN); %TCA+D_A(MN)*L(MN)*DRHOVT(MN); % Revised from ""(D_V(ML,ND)*Eta(ML,ND)+D_Vg(ML))*L(MN)*DRHOVT(MN)
        TCON(6)=5.2e-3*4.182; % ice thermal conductivity
        SF(6)=0.125;
        TARG(2)=0;%TCON(2)/TCON(1)-1;
        TARG(6)=TCON(6)/TCON(1)-1;
        GRAT(2)=0;%0.667/(1+TARG(2)*SF(2))+0.333/(1+TARG(2)*(1-2*SF(2)));
        ...
        if Theta_LL(ML,ND)==XXX
            ...
            EHCAP(ML,ND)=EHCAP(ML,ND)+HCAP(6)*XII; %(0.448*RHOV(MN)*4.182+HCAP(2))*Theta_V(ML,ND)+ The Calorie should be converted as J
        else
        ...
        TTGRAT(2)=0;%0.667/(1+TARG(2)*TSF(2))+0.333/(1+TARG(2)*(1-2*TSF(2)));
        ...
        TZETA(ML,ND)=0;%TTGRAT(2)/(TTGRAT(2)*(POR(J)-XXX)+XXX+TPS2(J));  % ita_T enhancement factor
        if Theta_LL(ML,ND)==XXX
            EHCAP(ML,ND)=HCD(J)+HCAP(1)*Theta_LL(ML,ND);
            EHCAP(ML,ND)=EHCAP(ML,ND)+HCAP(6)*XII; %+(0.448*RHOV(MN)*4.182+HCAP(2))*Theta_V(ML,ND) The Calorie should be converted as J
        else
